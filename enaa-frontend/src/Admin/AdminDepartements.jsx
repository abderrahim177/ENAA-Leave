...
export default function AdminDepartements() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://127.0.0.1:8000/api/departments', formData, {
        headers: { Authorization: `Bearer ${token}` }
      });
            ...
            <div key={d.id || idx} className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm space-y-3">
              ...
              <div>
                ...
                <p className="text-slate-400 text-[10px] mt-1">
                  Chef: <span className="text-slate-700 font-medium">
                    {d.manager ? d.manager.name : (d.manager_name || 'Non assigné')}
                  </span>
                </p>
              </div>
              <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-[10px] text-slate-500">
                ...
                <span className="font-bold text-slate-800">
                  {d.formateurs_count !== undefined ? `${d.formateurs_count} Formateurs` : (d.count || '0 Formateurs')}
                </span>
              </div>
            </div>
          ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-fadeIn">
          <div className="bg-white w-full max-w-md rounded-2xl shadow-xl border border-slate-100 overflow-hidden">
            ...
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
              <h3 className="font-bold text-slate-800 text-sm">Ajouter un nouveau Département</h3>
              <button 
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="space-y-1">
                <label className="text-[11px] font-semibold text-slate-700">Nom du Département</label>
