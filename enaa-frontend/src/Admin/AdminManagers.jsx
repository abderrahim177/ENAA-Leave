...
export default function AdminManagers() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://127.0.0.1:8000/api/add_manager', {
        ...formData,
        role: 'manager'
      }, {
        headers: { Authorization: `Bearer ${token}` }
      ...
      setFormData({ name: '', email: '', password: '', department_id: depts[0]?.id || '' });
    } catch (err) {
      setError(err.response?.data?.message || "Erreur lors de la création du compte manager.");
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="p-12 text-center text-slate-400 text-xs flex justify-center items-center gap-2">
          <Loader2 className="w-4 h-4 animate-spin" /> Chargement des managers...
        </div>
      ) : managers.length === 0 ? (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-fadeIn">
          <div className="bg-white w-full max-w-md rounded-2xl shadow-xl border border-slate-100 overflow-hidden">
            ...
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
              <h3 className="font-bold text-slate-800 text-sm">Nouveau Responsable / Manager</h3>
              <button 
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              {/* Email Input */}
              <div className="space-y-1">
                <label className="text-[11px] font-semibold text-slate-700">Adresse Email Pro</label>
                <div className="relative flex items-center">
              ...
              <div className="space-y-1">
                <label className="text-[11px] font-semibold text-slate-700">Département Attribué</label>
                <div className="relative flex items-center">
