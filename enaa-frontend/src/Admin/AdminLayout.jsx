...
export default function AdminLayout() {
  ...
  const navItems = [
    { name: "Vue d'ensemble", path: "/Admin_RH/dashboard", icon: PieChart },
    { name: "Gestion Formateurs", path: "/Admin_RH/formateurs", icon: Users },
    { name: "Gestion Managers", path: "/Admin_RH/managers", icon: UserCheck },
    { name: "Validation Congés", path: "/Admin_RH/conges", icon: FileCheck, badge: "5" },
    { name: "Départements", path: "/Admin_RH/departements", icon: Building2 },
    { name: "Paramètres RH", path: "/Admin_RH/settings", icon: Settings },
  ];
    ...
    <div className="h-screen w-screen overflow-hidden bg-slate-50 text-slate-800 font-sans flex flex-col text-xs">
      ...
      <header className="h-14 bg-white border-b border-slate-200 shrink-0 px-4 flex items-center justify-between shadow-sm z-30">
        <div className="flex items-center gap-3">
          ...
          <Link to="/Admin_RH" className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-lg bg-indigo-900 text-indigo-300 flex items-center justify-center font-bold text-xs shadow-sm">
              <ShieldCheck className="w-4 h-4" />
            </div>
            <span className="text-sm font-bold text-slate-900 tracking-tight">
              ENAA<span className="text-indigo-700">RH</span>{" "}
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-full bg-indigo-100 text-indigo-900 font-bold flex items-center justify-center border border-indigo-200">
              {adminInfo.nom ? adminInfo.nom[0] : "A"}
            </div>
            <div className="hidden sm:block text-left">
              <p className="font-semibold text-slate-900 text-[11px]">
                {adminInfo.nom || "Admin RH"}
              </p>
              <p className="text-[10px] text-slate-400">Ressources Humaines</p>
            </div>
          </div>
        </div>
      </header>
              ...
              {navItems.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    {item.badge && (
                      <span className="bg-indigo-100 text-indigo-800 font-bold text-[10px] px-1.5 py-0.5 rounded-full">
                        {item.badge}
                      </span>
                    )}
                  </NavLink>
